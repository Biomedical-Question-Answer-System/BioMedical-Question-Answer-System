...
public class AaeBIOQA {
  public static void main(String[] args) throws Exception {
    ...
    /*
     * InputStream stream = getClass().getResourceAsStream(filePath); try {
     * System.out.println("stream " + stream.read()); } catch (IOException e) { // TODO
     * Auto-generated catch block e.printStackTrace(); }
     */
    Object value = filePath;
    if (String.class.isAssignableFrom(value.getClass())) {
      inputs = TestSet.load(value.getClass().getResourceAsStream(String.class.cast(value)))
              .stream().collect(toList());
    } else if (String[].class.isAssignableFrom(value.getClass())) {
      inputs = Arrays.stream(String[].class.cast(value))
              .flatMap(path -> TestSet.load(value.getClass().getResourceAsStream(path)).stream())
              .collect(toList());
    }
